...
export default function StudentComplaintsHistoryPage() {
  ...
  return (
    <ProtectedRoute>
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 transition-colors duration-200 min-w-0">
        {/* Header */}
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 text-xs">
            ...
            {CATEGORIES.slice(0, 7).map((cat) => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-[11px] font-medium whitespace-nowrap transition-all duration-150 btn-interactive ${
                  filterCategory === cat
        ...
        {isLoading && complaints.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-slate-400">
            ...
            <p className="text-xs font-medium">Loading grievance history...</p>
          </div>
        ) : complaints.length === 0 ? (
          <div className="glass-panel p-12 rounded-3xl border border-slate-200 dark:border-slate-800 text-center shadow-sm animate-scale-in">
            ...
            <Link
              href="/complaints/new"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-bold text-xs shadow-md shadow-emerald-500/20 btn-interactive btn-primary-glow"
            >
              <PlusCircle className="w-4 h-4" />
              Submit a Complaint
            </Link>
          ...
          <div className="space-y-3">
            {complaints.map((item) => (
              <Link
                ...
                className="block glass-panel p-5 rounded-2xl border border-slate-200 dark:border-slate-800 hover:border-emerald-500/40 group card-lift transition-all shadow-sm"
              >
